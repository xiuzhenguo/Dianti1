...
@interface CKDianTiViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation CKDianTiViewController
...
{
    __weak typeof(self) weakSelf = self;
    [GoveCkManager getAlarmListWithVID:areaID pageindex:[NSString stringWithFormat:@"%ld",(long)self.pageNum] pagesize:@"10" success:^(id responseObject) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    CKDTDetailViewController *vc = [[CKDTDetailViewController alloc] init];
    GaoJingModel *model = self.itemArray[indexPath.row];
    vc.strLid = model.evevatorID;
